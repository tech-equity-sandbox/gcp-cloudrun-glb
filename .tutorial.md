...
You can change the modes by performing step `m`. 

Enter a preference by selecting an option from the list below:

- `y` to view a description of the configuration performed or commands executed in preview mode. Commands are displayed with a valid code.
- `n` to execute commands in the create mode.
- `d` to delete configured resources in the delete mode.

After selecting your preferred mode, enter any key to return to the main menu.

### Select project

You will use this project to deploy the resources required for this lab.

*Hint: This should be an existing project where you are an Owner, or have permissions to enable APIs. We recommend deleting this project to avoid incurring costs. Should you choose to retain the project, change to delete mode using step `m` in the menu and delete resources you do not wish to retain.*
...
Enter `m` at the prompt to sign-in and switch to project **{{project-id}}**.

To change to from preview mode to create mode, enter `n` at the prompt. 
...
Note the command line options specifying the platform as managed, the region where the service is deployed, an option allowing access by unauthenticated users, the image URL path and maximum number of instances. 
...
Note the command line options specifying the platform as managed, the region where the service is deployed, the option allowing access by unauthenticated users, the image URL path and maximum number of instances.
...
Note the command line options specifying the platform as managed, the region where the service is deployed, the option allowing access by unauthenticated users, the image URL path, the tag and maximum number of instances. 
...
Note the command line options specifying the platform as managed, the region where the service is deployed, the option allowing access by unauthenticated users, the image URL path, the version tag and maximum number of instances.
...
When you are done exploring the lab, enter `m` at the prompt to perform additional steps. 
...
We recommend starting from the last step performed and tracing your path through the lab backwards.